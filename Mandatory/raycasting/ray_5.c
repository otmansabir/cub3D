...
/*   Updated: 2024/05/11 16:27:13 by osabir           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		, double rayangle, int *top, int *bottom)
{
	...
	(*globel)->texture->distance_p = ((double)((X_WIDTH) / 2)) / tan((*globel)->g_player->fov_angle / 2);
	
	(*globel)->texture->wall_strip_height = ((double)(CUB_SIZE / (*globel)->texture->ray_distance)) * (*globel)->texture->distance_p;
...
void	render_3d_projecte_walls(t_globel **globel, int column, double rayangle)
{
	...
	while (i < bottom && i < Y_HEIGHT)
	{
		...
		// if ((*globel)->cast->its_hit_vertical)
		// 	my_put_pixel(globel, column, i++, 0xe4e4e3);
		// else
		// 	my_put_pixel(globel, column, i++, 0xb9c8d1);
	}
	while (i < Y_HEIGHT)
