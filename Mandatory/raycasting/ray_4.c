...
/*   Updated: 2024/05/06 19:02:34 by osabir           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_clalc_ver_hor(t_globel **globel, double *h, double *v)
{
	...
	if ((*globel)->cast->horizontal->found_horiz_wall_hit)
		(*h) = distance((*globel)->g_player->pos_x, (*globel)->g_player->pos_y,
				(*globel)->cast->horizontal->found_horiz_x, (*globel)->cast->horizontal->found_horiz_y);

	if (!(*globel)->cast->horizontal->found_horiz_wall_hit)
		(*h) = LONG_MAX;
		
	if ((*globel)->cast->vertical->found_vertic_wall_hit)
		(*v) = distance((*globel)->g_player->pos_x, (*globel)->g_player->pos_y,
				(*globel)->cast->vertical->found_vertic_x, (*globel)->cast->vertical->found_vertic_y);
